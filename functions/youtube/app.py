...
def lambda_handler(event, context):
    ...
    # S3에서 영상 파일 탐색 (규칙: {dday}_{event_name}.mp4)
    ...
    body = {
        "snippet": {
            "title": f"[Nextflow KR] {event_name} ({dday})",
            "description": f"Nextflow 한국어 커뮤니티 모임 영상\n행사일: {dday}",
            "tags": ["Nextflow", "bioinformatics", "워크플로", "한국어"],
    ...
    os.unlink(tmp_path)
    return {"statusCode": 200, "video_url": video_url}
